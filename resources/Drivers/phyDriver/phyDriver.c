...
int custom_port_phy_mdio_setup(struct custom_port_phy_platform_local *lp,
...
static int phy_platform_probe(struct platform_device *pdev) {
  ...
  lp = devm_kzalloc(&pdev->dev, sizeof(struct custom_port_phy_platform_local),
                    GFP_KERNEL);
  if (!lp) {
...
static struct phy_driver custom_phy_driver = {
    ...
    .phy_id = 0x00000002, .phy_id_mask = 0xffffffff, .name = "custom_port_phy",
...
static struct platform_driver custom_phy_platform_driver = {
    ...
    .driver =
        {
            .name = "custom_port_phy", .of_match_table = port_phy_of_match,
        },
#endif
};
